...
with tab_fleet:
    ...
    def snapshot_index(engine_df: pd.DataFrame) -> int:
        target_cycle = max(1, int(np.ceil(engine_df["cycle"].max() * snapshot_pct / 100)))
        return (engine_df["cycle"] - target_cycle).abs().idxmin()

    snapshot_idx = df.groupby("engine_id")["cycle"].apply(
        lambda cycles: snapshot_index(df.loc[cycles.index])
    )
    latest = df.loc[snapshot_idx.values].reset_index(drop=True).copy()
